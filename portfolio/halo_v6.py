#!/usr/bin/env python3
"""
HALO v6.1 — VAA Korean Edition (bugfix)
...
ETF 유니버스:
  ...
  [방어] 214980 KODEX 단기채권PLUS    (1순위)
         411060 ACE KRX금현물          (2순위)
         439870 KODEX 국고채30년       (3순위)  ← v6.0의 329750(미검증) 대신 실보유 종목
...
def generate_orders_buy_only(
    ...
    # shortfall 기반 잔여 현금 배분 (target_w > 0 인 종목만 대상)
    target_mask = df["target_w"] > 0
    if target_mask.any():
        min_target_price = float(df.loc[target_mask, "price"].min())
    else:
        min_target_price = float("inf")

    while remaining >= min_target_price:
        aff = df[(df["price"] <= remaining) & (df["target_w"] > 0)].copy()
        if aff.empty:
            break
...
def build_email(d: dict) -> str:
    ...
    def _fmt_score(s: float) -> str:
        if not (s == s) or s == float("-inf"):   # nan or -inf
            return "▼ N/A"
        return f"{'▲' if s >= 0 else '▼'} {s*100:.2f}%"
    ...
    score_rows = "".join(
        ...
        f"{_fmt_score(s)}</td></tr>"
        for t, s in sorted(d["scores"].items(), key=lambda x: x[1], reverse=True)
    )
